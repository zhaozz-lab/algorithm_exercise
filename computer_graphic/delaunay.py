...
class Delaunay2d:
  ...
  def show(self, width=500, height=500, showVertices=False, showCells=False, showContour=[]):
    ...
    for e in self.edge2Triangles:
      ...
      cv2.line(image,(xp1,yp1),(xp2,yp2),(255,255,255))
    ...
    if showVertices:
      for i in range(len(self.points)):
        ...
        cv2.circle(image,(xp,yp),5,(255,0,0))
    
    if showCells:
      for tId, tVals in enumerate(self.triangles):
    ...
    if len(showContour) > 0:
      for i in range(len(showContour) - 1):
        ...
        cv2.line(image,(xp1,yp1),(xp2,yp2),(0,0,255))
...
if __name__ == '__main__':
  ...
  # 相关paper http://www.s-hull.org/paper/s_hull.pdf
  import random 

  random.seed(1234)
  import numpy
  xyPoints = [numpy.array([random.random(), random.random()]) for i in range(20)]
  # print(xy)
  delaunay = Delaunay2d(xyPoints)
  delaunay.show(showVertices=True, showCells=True,showContour=[])